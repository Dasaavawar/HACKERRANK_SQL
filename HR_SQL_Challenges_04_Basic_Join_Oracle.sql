/* HackerRank - SQL - Challenges - IV - Basic Join - Oracle */
...
FROM Hackers H
JOIN (SELECT S.hacker_id, COUNT(C.challenge_id) AS COUNT
      FROM Challenges C JOIN Submissions S ON C.challenge_id=S.challenge_id JOIN Difficulty D ON C.difficulty_level=D.difficulty_level WHERE S.score/D.score=1 GROUP BY S.hacker_id HAVING COUNT(C.challenge_id)>=2)
X ON X.hacker_id=H.hacker_id
...
WITH T1 AS 
(select H.hacker_id, H.name, COUNT(challenge_id) AS total_challenges
FROM Hackers H
JOIN Challenges C ON C.hacker_id = H.hacker_id
GROUP BY H.hacker_id, H.name
ORDER BY total_challenges DESC, H.hacker_id ASC),
T2 AS
(SELECT COUNT(T1.hacker_id) AS num, T1.total_challenges
FROM T1
HAVING (COUNT(T1.hacker_id)>1 AND T1.total_challenges < (SELECT MAX(T1.total_challenges) FROM T1))
GROUP BY T1.total_challenges),
T3 AS
(SELECT T1.hacker_id, T1.name, T1.total_challenges
FROM T1
RIGHT JOIN T2 ON T2.total_challenges = T1.total_challenges)
SELECT T1.hacker_id, T1.name, T1.total_challenges
FROM T1
WHERE T1.hacker_id NOT IN (SELECT T3.hacker_id FROM T3);
...
SELECT A.hacker_id, A.name, SUM(A.MAX_SCORE)
FROM MAXCHALL A
